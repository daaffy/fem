...
stress_a = as_tensor(Expression((('pi*sin(pi*(x[0]+x[1]))+2*pi*cos(pi*x[0])*sin(pi*x[1])','pi*sin(pi*(x[0]+x[1]))'),('pi*sin(pi*(x[0]+x[1]))','pi*sin(pi*(x[0]+x[1]))+2*pi*sin(pi*x[0])*cos(pi*x[1])')), degree=2))
...
def _t_s(trial,test):
    sigma = trial[0]
    psi = test[3]
    return dot(sigma(sgn_2)*n(sgn_1),psi(sgn_2))*dS
...
a = [[A_solid(trial,test),0,-_t_s(trial,test)],
    [0,A_fluid(trial,test),_t_f(trial,test)],
    [-_t_s(test,trial), -_t_f(test,trial), 0]]

l =  [elastic_load(test),0,0]
