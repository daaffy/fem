...
_H = FunctionSpace(mesh, 'RT', 2) 
# P1 = FunctionSpace(mesh, "RT", 1) # (!) Use FiniteElement??? See UFL doc sheet
R = FunctionSpace(mesh, "CG", 1)
P2 = FunctionSpace(mesh, "CG", 1)
P3 = VectorFunctionSpace(mesh, "CG", 1)

V = BlockFunctionSpace([_H, _B, _H, _B, R, P2, P3], restrict=[
    solid_restriction, solid_restriction, 
    solid_restriction, solid_restriction, 
    solid_restriction, 
    fluid_restriction,
...
(sig1_H, pre_sig1_B, sig2_H, pre_sig2_B, r, p, phi) = block_split(trial_func)

test_func = BlockTestFunction(V)
(tau1_H, pre_tau1_B, tau2_H, pre_tau2_B, s, q, psi) = block_split(test_func)
...
a = [[s11(sig1_H,tau1_H,invC_11),    s11(sig1_B,tau1_H,invC_11),    s12(sig2_H,tau1_H,invC_12),     s12(sig2_B,tau1_H,invC_12),     -r1(tau1_H,r,1),    0,          w1(tau1_H,phi)  ],
     [s11(sig1_H,tau1_B,invC_11),    s11(sig1_B,tau1_B,invC_11),    s12(sig2_H,tau1_B,invC_12),     s12(sig2_B,tau1_B,invC_12),     -r1(tau1_B,r,1),    0,          w1(tau1_B,phi)  ],
     [s12(sig1_H,tau2_H,invC_21),    s12(sig1_B,tau2_H,invC_21),    s11(sig2_H,tau2_H,invC_22),     s11(sig2_B,tau2_H,invC_22),     r1(tau2_H,r,0),     0,          w2(tau2_H,phi)  ],
     [s12(sig1_H,tau2_B,invC_21),    s12(sig1_B,tau2_B,invC_21),    s11(sig2_H,tau2_B,invC_22),     s11(sig2_B,tau2_B,invC_22),     r1(tau1_B,r,0),     0,          w2(tau2_B,phi)  ],
     [-omega**2*r1(sig1_H,s,1),      -omega**2*r1(sig1_B,s,1),      omega**2*r1(sig2_H,s,0),        omega**2*r1(sig2_B,s,0),        0,                  0,          0               ],
     [0,                             0,                             0,                              0,                              0,                  t1(p,q),    w3(q,phi)       ],
     [w1(sig1_H,psi),                w1(sig1_B,psi),                w2(sig2_H,psi),                 w2(sig2_B,psi),                 0,                  w3(p,psi),  0               ]]
...
#      [s12(sig1_H,tau2_B,invC_21), s12(sig1_B,tau2_B,invC_21), s11(sig2_H,tau2_B,invC_22), s11(sig2_B,tau2_B,invC_22), r1(tau2_B,r,0)],
#      [-r1(sig1_H,s,1), -r1(sig1_B,s,1), r1(sig2_H,s,0), r1(sig2_B,s,0), 0]]
...
#   pressure_python[i] = U[5](x)
#   # plotvals[i] = pressure_python[i] - pressure_a(x)
#   plotvals[i] = pressure_python[i]
